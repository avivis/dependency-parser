...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        if len(state.buffer) == 0:
            ...
            word=words[state.buffer[-1]]
            partofspeech=pos[state.buffer[-1]]
            if word is None:
                input_rep.append(self.word_vocab['<NULL>'])
            elif partofspeech == 'CD':
                input_rep.append(self.word_vocab['<CD>'])
            ...
            for j in range(1,3):
                word=words[state.buffer[-j]]
                partofspeech=pos[state.buffer[-j]]
                if word is None:
                    input_rep.append(self.word_vocab['<ROOT>'])
                elif partofspeech == 'CD':
                    input_rep.append(self.word_vocab['<CD>'])
    ...
    def get_output_representation(self, output_pair):  
        output = np.zeros(91)
        output[self.output_labels.get(output_pair)] = 1
        return output
